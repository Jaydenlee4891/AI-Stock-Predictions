...
def fetch_portfolio():
    ...
    for pos in positions:
        portfolio.append({
            ...
            'unrealized_pl': pos.unrealized_pl,  # Fixed: 'unrealized_p1' -> 'unrealized_pl'
            'side': 'buy'
        })
    return portfolio
...
Now, answer the following question using this background: {message}
"""
...
class TradingBotGUI:
    def __init__(self, root):
        ...
        self.chat_output = tk.Text(root, height=10, width=70, state=tk.DISABLED)
    ...
    def get_max_entry_price(self, symbol):
        try:
            ...
            prices = [float(order.filled_avg_price) for order in orders if order.symbol == symbol and order.filled_avg_price]
    ...
    def trade_systems(self):
        for symbol, data in self.equities.items():
            if data['status'].lower() == 'on':
                ...
                level_prices = {i + 1: round(entry_price * (1 - data['drawdown'] * (i + 2)), 2)
    ...
    def place_order(self, symbol, price, level):
        if -level in self.equities[symbol]['levels'] or str(-level) in self.equities[symbol]['levels']:
            return
        try:
            api.submit_order(
                symbol=symbol,
